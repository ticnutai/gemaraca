...
function extractSections(text: string): PsakSection[] {
  ...
  const sectionPatterns: Array<{ type: PsakSection['type']; patterns: RegExp[] }> = [
    { type: 'summary', patterns: [/^תקציר[:\s]/m, /^סיכום\s*(?:פסק\s*הדין)?[:\s]/m] },
    { type: 'facts', patterns: [
      /^עובדות(?:\s*(?:המקרה|התיק|הרקע))?[:\s]/m,
      /^רקע(?:\s*עובדתי)?[:\s]/m,
      /^המקרה\s*העובדתי[:\s]/m,
      /^תיאור\s*(?:המקרה|העובדות)[:\s]/m,
    ] },
    { type: 'chapters', patterns: [/^ראשי\s*פרקים[:\s]?/m, /^תוכן\s*(?:ה)?עניינים[:\s]?/m] },
    { type: 'plaintiff-claims', patterns: [/^טענות\s*התובע[ים]?[:\s]/m, /^טענות\s*צד\s*א[:\s]/m, /^טענות\s*המבקש[ים]?[:\s]/m] },
    { type: 'defendant-claims', patterns: [/^טענות\s*הנתבע[ים]?[:\s]/m, /^טענות\s*צד\s*ב[:\s]/m, /^טענות\s*המשיב[ים]?[:\s]/m] },
    { type: 'discussion', patterns: [/^דיון[:\s]/m, /^דיון\s*(?:הלכתי|משפטי)[:\s]/m, /^ניתוח\s*(?:הלכתי|משפטי)[:\s]/m] },
    { type: 'reasoning', patterns: [/^הנמקה[:\s]/m, /^נימוקים[:\s]/m, /^נימוקי\s*(?:הדין|הפסק|בית\s*הדין)[:\s]/m] },
    { type: 'law-sources', patterns: [/^מקורות\s*(?:הלכתיים|משפטיים)?[:\s]/m, /^מראי\s*מקומות[:\s]/m] },
    { type: 'ruling', patterns: [/^פסק\s*הדין[:\s]/m, /^פסק[:\s]/m, /^פסיקה[:\s]/m] },
    { type: 'decision', patterns: [/^החלטה[:\s]/m, /^הכרעת\s*הדין[:\s]/m, /^הכרעה[:\s]/m, /^מסקנה[:\s]/m, /^מסקנות[:\s]/m] },
    { type: 'conclusion', patterns: [/^סוף\s*דבר[:\s]/m, /^לסיכום[:\s]/m, /^סיכום[:\s]/m] },
  ];
